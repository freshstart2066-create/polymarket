...
+ BONUS: Wallet balance checker (native token + USDC, multi-network)
"""
...
# Wallet lookup — tracks users mid-flow (waiting for network reply)
...
def erc20_balance_calldata(wallet: str) -> str:
    """Build balanceOf(address) calldata for an ERC-20 contract"""
...
async def get_wallet_balances(wallet: str, network_key: str) -> str:
    """Fetch native + USDC + USDT balances and return formatted message"""
    ...
    try:
        ...
        # USDT balance (not all networks have it)
        usdt_line = ""
        if net.get("usdt"):
...
async def cmd_wallet(chat_id: str, args: list):
    ...
    if len(args) >= 2:
        ...
        # Address provided — ask for network
        wallet_addr = args[0]
        if not wallet_addr.startswith("0x") or len(wallet_addr) < 40:
            await send(chat_id, "❌ Invalid address format.\n\nUsage: `/wallet 0xYourAddress polygon`")
            return
        ...
        await send(chat_id, (
            "💼 *Wallet Balance Checker*\n\n"
            "Check native token + USDC + USDT balances on any network.\n\n"
            "*Usage:*\n"
        ...
        while True:
            try:
                ...
                for update in data.get("result", []):
                    ...
                    # ── Handle pending wallet network reply (free text) ──
                    if chat_id in wallet_pending and not raw_text.startswith("/"):
                        network_key = raw_text.lower()
...
async def detect_sentiment_shift(market_id: str, current_ratio: float, ts: str):
    """Detect when market sentiment reverses"""
...
async def main():
    ...
    await send(OWNER_CHAT_ID, (
        ...
        "📈 ASCII Charts\n"
        "💼 Wallet Balance Checker\n\n"
        "/wallet 0xAddress polygon — check wallet\n"
